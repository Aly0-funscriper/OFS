    ...
    keys->RegisterGroup("Utility", Tr::UTILITY_BINDING_GROUP);
    {
		keys->RegisterAction(
			{ "toggle_rectangle_selection",
				[this]() { scriptTimeline.ToggleBoxSelectionMode(); },
				false },
			"Toggle rectangle selection mode", "Utility",
        ...
        keys->RegisterAction(
            { "select_all",
                [this]() {
					scriptTimeline.InvalidateBoxSelectionPaste();
...
void OpenFunscripter::quickExport() noexcept
{
    ...
    generateExportGraphsForDirectory(Util::PathFromString(LoadedProject->Path()));
}
...
void OpenFunscripter::addEditAction(int pos) noexcept
{
	...
	if (ActiveFunscript()->HasSelection()) {
		auto actions = ActiveFunscript()->Actions();
		auto selection = ActiveFunscript()->Selection();
		for (auto& action : actions) {
			if (selection.find(action) != selection.end()) {
				action.pos = pos;
			}
		}
		...
		ActiveFunscript()->SetActions(actions);
		ActiveFunscript()->SetSelection(selection);
		return;
	}
...
void OpenFunscripter::pasteSelection() noexcept
{
	...
	// Multi-anchor paste is armed only by the most recent right-button box
	// selection. Left-button selections always retain OFS's original paste.
	if (scriptTimeline.IsBoxSelectionPasteEligible(ActiveFunscript())) {
		const auto anchors = ActiveFunscript()->Selection();
		auto actions = ActiveFunscript()->Actions();
		const float copiedStartTime = CopiedSelection.front().atS;
		const float duration = player->Duration();

		for (const auto& anchor : anchors) {
			for (const auto& copiedAction : CopiedSelection) {
				const float pasteTime = anchor.atS + (copiedAction.atS - copiedStartTime);
				if (pasteTime < 0.f || pasteTime > duration) continue;

				FunscriptAction pastedAction(pasteTime, copiedAction.pos, copiedAction.tag);
				auto existingAction = actions.find(pastedAction);
				if (existingAction != actions.end()) {
					*existingAction = pastedAction;
				}
				else {
					actions.emplace(pastedAction);
				}
			}
		}

		FunscriptArray updatedAnchors;
		for (const auto& anchor : anchors) {
			auto updatedAnchor = actions.find(anchor);
			if (updatedAnchor != actions.end()) {
				updatedAnchors.emplace(*updatedAnchor);
			}
		}
		ActiveFunscript()->SetActions(actions);
		ActiveFunscript()->SetSelection(updatedAnchors);
		return;
	}
    ...
    Util::SaveFileDialog(TR(SAVE),
        ...
        [this](auto& result) {
            if (result.files.size() > 0) {
                ...
                dir.remove_filename();
                generateExportGraphsForDirectory(dir);
                auto& ofsState = OpenFunscripterState::State(stateHandle);
...
void OpenFunscripter::generateExportGraphsForDirectory(const std::filesystem::path& directory) noexcept
{
#ifdef _WIN32
    ...
    const auto pathLength = GetModuleFileNameW(nullptr, modulePath.data(), static_cast<DWORD>(modulePath.size()));
    if (pathLength == 0 || pathLength == modulePath.size()) {
        return;
    }

    modulePath.resize(pathLength);
    const auto executable = std::filesystem::path(modulePath).parent_path() / L"heatmapgen.exe";
    if (!std::filesystem::is_regular_file(executable)) {
        MessageBoxW(nullptr, L"找不到 heatmapgen.exe，无法生成导出图片。", L"OpenFunscripter", MB_OK | MB_ICONWARNING);
        return;
    }

    std::wstring commandLine = L"\"" + executable.wstring() + L"\" \"" + directory.wstring() + L"\"";
    ...
    if (!CreateProcessW(executable.c_str(), commandLine.data(), nullptr, nullptr, FALSE, CREATE_NO_WINDOW, nullptr,
        ...
        MessageBoxW(nullptr, L"无法启动 heatmapgen.exe，无法生成导出图片。", L"OpenFunscripter", MB_OK | MB_ICONWARNING);
        return;
    }
                        ...
                        Util::OpenDirectoryDialog(TR(EXPORT_MENU), ofsState.lastPath,
                            [this](auto& result) {
                                if (result.files.size() > 0) {
                                    ...
                                    generateExportGraphsForDirectory(Util::PathFromString(result.files[0]));
                                }
                            });
                    }
                }
