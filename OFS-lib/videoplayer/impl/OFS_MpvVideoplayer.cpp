...
bool OFS_Videoplayer::Init(bool hwAccel) noexcept
{
	...
	// Preload compressed packets ahead of the current position. Forward playback
	// gets roughly 30 seconds of read-ahead. Keep a compact backward reserve
	// (about 20 frames for typical files) for responsive stepping both ways.
	error = mpv_set_option_string(CTX->mpv, "cache", "yes");
	if (error != 0) {
		LOG_WARN("Failed to set mpv: cache=yes");
	}
	error = mpv_set_option_string(CTX->mpv, "cache-secs", "30");
	if (error != 0) {
		LOG_WARN("Failed to set mpv: cache-secs=30");
	}
	error = mpv_set_option_string(CTX->mpv, "demuxer-readahead-secs", "30");
	if (error != 0) {
		LOG_WARN("Failed to set mpv: demuxer-readahead-secs=30");
	}
	error = mpv_set_option_string(CTX->mpv, "demuxer-max-bytes", "536870912");
	if (error != 0) {
		LOG_WARN("Failed to set mpv: demuxer-max-bytes=536870912");
	}
	error = mpv_set_option_string(CTX->mpv, "demuxer-max-back-bytes", "33554432");
	if (error != 0) {
		LOG_WARN("Failed to set mpv: demuxer-max-back-bytes=33554432");
	}
	error = mpv_set_option_string(CTX->mpv, "cache-pause-initial", "no");
	if (error != 0) {
		LOG_WARN("Failed to set mpv: cache-pause-initial=no");
	}
	...
	mpv_observe_property(CTX->mpv, MpvCacheDuration, "demuxer-cache-duration", MPV_FORMAT_DOUBLE);
            ...
            case MPV_EVENT_PROPERTY_CHANGE:
            {
                ...
                switch (mp_event->reply_userdata) {
                    ...
                    case MpvFramesPerSecond:
                        ...
                        break;
                    case MpvCacheDuration:
                        ctx->data.cacheDuration = *(double*)prop->data;
                        break;
                    case MpvDuration:
...
{
    if (IsPaused()) {
        // Keep the displayed frame and OFS's logical timeline on the same
        // exact-seek path used by PreviousFrame. mpv's asynchronous
        // frame-step can lag behind repeated key presses while the logical
        // position continues advancing, which makes the two frame counts
        // diverge.
        double relSeek = FrameTime() * 1.000001;
		logicalPosition += (relSeek / CTX->data.duration);
		logicalPosition = Util::Clamp(logicalPosition, 0.f, 1.f);
		SetPositionPercent(logicalPosition, false);
    }
}
...
void OFS_Videoplayer::PreviousFrame() noexcept
{
    if (IsPaused()) {
        double relSeek = FrameTime() * 1.000001;
		// Preserve OFS's original exact-seek backstep behavior so the displayed
		// frame always matches the logical timeline position. Base consecutive
		// commands on logicalPosition because percent-pos updates asynchronously.
		logicalPosition -= (relSeek / CTX->data.duration);
		logicalPosition = Util::Clamp(logicalPosition, 0.f, 1.f);
		SetPositionPercent(logicalPosition, false);
    }
...
void OFS_Videoplayer::SeekFrames(int32_t offset) noexcept
{
    ...
    if (IsPaused()) {
        ...
        // percentPos is reported asynchronously by mpv and may still refer to
        // an older seek. Base repeated frame jumps on OFS's requested logical
        // position so every key press advances from the previous target.
        logicalPosition += (relSeek / CTX->data.duration);
        logicalPosition = Util::Clamp(logicalPosition, 0.f, 1.f);
        SetPositionPercent(logicalPosition, false);
    }
...
double OFS_Videoplayer::CacheDuration() const noexcept
{
    return CTX->data.cacheDuration;
}
