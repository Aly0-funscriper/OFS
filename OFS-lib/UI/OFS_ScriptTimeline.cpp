...
void ScriptTimeline::applyBoxSelection(const OverlayDrawingCtx& ctx) noexcept
{
	OFS_PROFILE(__FUNCTION__);
	const float startTime = std::min(BoxSelectionStartAction.atS, BoxSelectionEndAction.atS);
	const float endTime = std::max(BoxSelectionStartAction.atS, BoxSelectionEndAction.atS);
	const int32_t minPos = std::min(BoxSelectionStartAction.pos, BoxSelectionEndAction.pos);
	const int32_t maxPos = std::max(BoxSelectionStartAction.pos, BoxSelectionEndAction.pos);

	auto script = ctx.DrawingScript();
	FunscriptArray selection = BoxSelectionAdditive ? script->Selection() : FunscriptArray{};
	auto action = script->Actions().lower_bound(FunscriptAction(startTime, 0));
	for (; action != script->Actions().end() && action->atS <= endTime; ++action) {
		if (action->pos >= minPos && action->pos <= maxPos) {
			selection.emplace(*action);
		}
	}
	script->SetSelection(selection);
	BoxSelectionPasteArmed = !selection.empty();
	LastBoxSelectionScript = script;
	LastBoxSelectionAnchors = selection;
}

void ScriptTimeline::InvalidateBoxSelectionPaste() noexcept
{
	BoxSelectionPasteArmed = false;
	LastBoxSelectionScript.reset();
	LastBoxSelectionAnchors.clear();
}

bool ScriptTimeline::IsBoxSelectionPasteEligible(const std::shared_ptr<Funscript>& script) const noexcept
{
	if (!BoxSelectionPasteArmed) return false;
	auto boxSelectionScript = LastBoxSelectionScript.lock();
	if (!boxSelectionScript || boxSelectionScript != script) return false;
...
void ScriptTimeline::ToggleBoxSelectionMode() noexcept
{
	BoxSelectionModeEnabled = !BoxSelectionModeEnabled;
	if (!BoxSelectionModeEnabled) {
		IsBoxSelecting = false;
		BoxSelectingScriptIdx = -1;
		InvalidateBoxSelectionPaste();
	}
}
...
void ScriptTimeline::handleBoxSelectionScrolling(const OverlayDrawingCtx& ctx) noexcept
{
	if (!IsBoxSelecting || !BoxSelectionMoved || !ImGui::IsMouseDown(ImGuiMouseButton_Right)) return;

	const float edgeMargin = std::max(24.f, ctx.canvasSize.x * 0.03f);
	const float leftEdge = ctx.canvasPos.x + edgeMargin;
	const float rightEdge = ctx.canvasPos.x + ctx.canvasSize.x - edgeMargin;
	const float mouseX = ImGui::GetMousePos().x;
	float direction = 0.f;
	float pressure = 0.f;
	if (mouseX < leftEdge) {
		direction = -1.f;
		pressure = Util::Clamp((leftEdge - mouseX) / edgeMargin, 0.f, 1.f);
	}
	else if (mouseX > rightEdge) {
		direction = 1.f;
		pressure = Util::Clamp((mouseX - rightEdge) / edgeMargin, 0.f, 1.f);
	}

	if (direction != 0.f) {
		const float centerTime = ctx.offsetTime + (ctx.visibleTime / 2.f);
		const float seekToTime = Util::Max(0.f,
			centerTime + direction * ctx.visibleTime * pressure * ImGui::GetIO().DeltaTime);
		EV::Enqueue<ShouldSetTimeEvent>(seekToTime);
	}
}
...
bool ScriptTimeline::handleTimelineClicks(const OverlayDrawingCtx& ctx) noexcept
{
	...
	auto rightMouseClicked = ImGui::IsMouseClicked(ImGuiMouseButton_Right);
	// Any left-button selection gesture switches back to OFS's original paste
	// behavior, even if it happens to select the same points as the last box.
	if (leftMouseClicked) {
		InvalidateBoxSelectionPaste();
	}
	...
	else if (rightMouseClicked && BoxSelectionModeEnabled)
	{
		if (ctx.drawingScriptIdx != ctx.activeScriptIdx) {
			EV::Enqueue<ShouldChangeActiveScriptEvent>(ctx.drawingScriptIdx);
		}
		IsBoxSelecting = true;
		BoxSelectionMoved = false;
		BoxSelectionAdditive = ImGui::IsKeyDown(ImGuiMod_Ctrl);
		BoxSelectingScriptIdx = ctx.drawingScriptIdx;
		BoxSelectionStart = mousePos;
		BoxSelectionEnd = mousePos;
		BoxSelectionStartAction = getActionForPoint(ctx, mousePos);
		BoxSelectionEndAction = BoxSelectionStartAction;
		return true;
	}
	return false;
}
	...
	int activeScriptIdx) noexcept
{
	...
	const double cachedSeconds = Util::Clamp(player->CacheDuration(), 0.0, 30.0);
	ImGui::Begin(BoxSelectionModeEnabled
		? FMT("%s | RECT SELECT [Alt+R] | CACHE %.0fs / 30s###%s", TR(POSITIONS), cachedSeconds, WindowId)
		: FMT("%s | CACHE %.0fs / 30s###%s", TR(POSITIONS), cachedSeconds, WindowId));
	...
	for(int i=0; i < scripts.size(); i += 1) 
	{
		...
		// Right-drag selection is constrained to this script's canvas. The
		// horizontal axis maps to time and the vertical axis maps to position.
		if (IsBoxSelecting && i == BoxSelectingScriptIdx) {
			auto mousePos = ImGui::GetMousePos();
			BoxSelectionEnd.x = Util::Clamp(mousePos.x, drawingCtx.canvasPos.x, drawingCtx.canvasPos.x + drawingCtx.canvasSize.x);
			BoxSelectionEnd.y = Util::Clamp(mousePos.y, drawingCtx.canvasPos.y, drawingCtx.canvasPos.y + drawingCtx.canvasSize.y);
			const ImVec2 dragDelta = mousePos - BoxSelectionStart;
			const float dragThreshold = ImGui::GetIO().MouseDragThreshold;
			BoxSelectionMoved = BoxSelectionMoved ||
				((dragDelta.x * dragDelta.x + dragDelta.y * dragDelta.y) >= dragThreshold * dragThreshold);
			BoxSelectionEndAction = getActionForPoint(drawingCtx, BoxSelectionEnd);
			handleBoxSelectionScrolling(drawingCtx);

			if (BoxSelectionMoved) {
				const auto startPoint = BaseOverlay::GetPointForAction(drawingCtx, BoxSelectionStartAction);
				ImVec2 boxMin(std::min(startPoint.x, BoxSelectionEnd.x), std::min(startPoint.y, BoxSelectionEnd.y));
				ImVec2 boxMax(std::max(startPoint.x, BoxSelectionEnd.x), std::max(startPoint.y, BoxSelectionEnd.y));
				drawingCtx.drawList->AddRectFilled(boxMin, boxMax, selectColorBackground);
				drawingCtx.drawList->AddRect(boxMin, boxMax, selectColor, 0.f, ImDrawFlags_None, 2.f);
			}

			if (ImGui::IsMouseReleased(ImGuiMouseButton_Right)) {
				if (BoxSelectionMoved) {
					applyBoxSelection(drawingCtx);
				}
				IsBoxSelecting = false;
				BoxSelectingScriptIdx = -1;
			}
		...
		// The original right-click context menu has been retired. Right-click is
		// reserved for rectangle selection in this timeline.
		if (false)
		{
