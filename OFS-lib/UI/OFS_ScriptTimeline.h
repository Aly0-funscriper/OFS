...
private:
	...
	void handleBoxSelectionScrolling(const OverlayDrawingCtx& ctx) noexcept;
	...
	void applyBoxSelection(const OverlayDrawingCtx& ctx) noexcept;
	...
	bool BoxSelectionAdditive = false;
	bool BoxSelectionModeEnabled = true;
	int32_t BoxSelectingScriptIdx = -1;
	ImVec2 BoxSelectionStart = ImVec2(0.f, 0.f);
	ImVec2 BoxSelectionEnd = ImVec2(0.f, 0.f);
	FunscriptAction BoxSelectionStartAction;
	FunscriptAction BoxSelectionEndAction;
	bool BoxSelectionPasteArmed = false;
	std::weak_ptr<Funscript> LastBoxSelectionScript;
	FunscriptArray LastBoxSelectionAnchors;
...
public:
	...
	void ToggleBoxSelectionMode() noexcept;
	inline bool IsBoxSelectionModeEnabled() const noexcept { return BoxSelectionModeEnabled; }
	void InvalidateBoxSelectionPaste() noexcept;
	bool IsBoxSelectionPasteEligible(const std::shared_ptr<Funscript>& script) const noexcept;
